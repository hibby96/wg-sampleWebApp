...
const SmartBuffer_1 = require("./SmartBuffer");
const definitions_1 = require("../definitions");
const encode_binaryarray_1 = require("../util/encode-binaryarray");
const __1 = require("..");
var DataFormat = protocol_1.Protocol.DataFormat;
const StaticFlushBuffer = Buffer.from([protocol_1.Protocol.FrontendMessageCode.Flush, 0x00, 0x00, 0x00, 0x04]);
const StaticTerminateBuffer = Buffer.from([protocol_1.Protocol.FrontendMessageCode.Terminate, 0x00, 0x00, 0x00, 0x04]);
const StaticSyncBuffer = Buffer.from([protocol_1.Protocol.FrontendMessageCode.Sync, 0x00, 0x00, 0x00, 0x04]);
class Frontend {
    constructor() {
        this._io = new SmartBuffer_1.SmartBuffer();
    }
    ...
    getStartupMessage(args) {
        const io = this._io.start()
            ...
            .writeInt16BE(protocol_1.Protocol.VERSION_MAJOR)
            .writeInt16BE(protocol_1.Protocol.VERSION_MINOR);
    ...
    getPasswordMessage(password) {
        const io = this._io.start()
            .writeInt8(protocol_1.Protocol.FrontendMessageCode.PasswordMessage)
    ...
    getParseMessage(args) {
        ...
        const io = this._io.start()
            .writeInt8(protocol_1.Protocol.FrontendMessageCode.Parse)
            .writeInt32BE(0) // Preserve header
    ...
    getBindMessage(args) {
        ...
        if (args.statement && args.statement.length > 63)
            throw new Error('Query name length must be lower than 63');
        const io = this._io.start()
            .writeInt8(protocol_1.Protocol.FrontendMessageCode.Bind)
            .writeInt32BE(0) // Preserve header
        ...
        const columnFormat = queryOptions.columnFormat != null ? queryOptions.columnFormat :
            definitions_1.DEFAULT_COLUMN_FORMAT;
            ...
            for (let i = 0; i < params?.length; i++) {
                ...
                if (dt) {
                    if (typeof dt.encodeBinary === 'function') {
                        ...
                        io.buffer.writeInt16BE(protocol_1.Protocol.DataFormat.binary, formatOffset + (i * 2));
                        // Preserve data length
                        io.writeInt32BE(0);
                        const dataOffset = io.offset;
                        if (dt.elementsOID) { // If data type is array
                            v = Array.isArray(v) ? v : [v];
                            (0, encode_binaryarray_1.encodeBinaryArray)(io, v, dt.elementsOID, queryOptions, dt.encodeBinary);
                        }
                        else {
                        ...
                        v = dt.elementsOID ?
                            (0, __1.stringifyArrayLiteral)(v, queryOptions, dt.encodeText) :
    ...
    getDescribeMessage(args) {
        ...
        const io = this._io.start()
            .writeInt8(protocol_1.Protocol.FrontendMessageCode.Describe)
            .writeInt32BE(0) // Preserve header
    ...
    getExecuteMessage(args) {
        ...
        const io = this._io.start()
            .writeInt8(protocol_1.Protocol.FrontendMessageCode.Execute)
            .writeInt32BE(0) // Preserve header
            .writeCString(args.portal || '', 'utf8')
    ...
    getCloseMessage(args) {
        if (args.name && args.name.length > 63)
            throw new Error(args.type === 'P' ? 'Portal' : 'Statement' +
                'name length must be lower than 63');
        const io = this._io.start()
            .writeInt8(protocol_1.Protocol.FrontendMessageCode.Close)
            .writeInt32BE(0) // Preserve header
            .writeUInt8(args.type.charCodeAt(0))
            .writeCString(args.name || '', 'utf8');
        return setLengthAndFlush(io, 1);
    ...
    getQueryMessage(sql) {
        const io = this._io.start()
            .writeInt8(protocol_1.Protocol.FrontendMessageCode.Query)
            .writeInt32BE(0) // Preserve header
