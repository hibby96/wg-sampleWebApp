...
class PgSocket extends SafeEventEmitter_1.SafeEventEmitter {
    constructor(options) {
        super();
        this.options = options;
        this._state = definitions_1.ConnectionState.CLOSED;
        this._backend = new Backend_1.Backend();
    ...
    connect() {
        if (this._socket)
            return;
        this._state = definitions_1.ConnectionState.CONNECTING;
    ...
    close() {
        ...
        if (this._state === definitions_1.ConnectionState.CLOSING)
            return;
        const socket = this._socket;
        this._state = definitions_1.ConnectionState.CLOSING;
        this._removeListeners();
    ...
    _handleConnect() {
        ...
        if (!socket)
            return;
        this._state = definitions_1.ConnectionState.AUTHORIZING;
        this._reset();
    ...
    _handleError(err) {
        if (this._state !== definitions_1.ConnectionState.READY) {
            this._socket?.end();
        }
        this.emit('error', err);
    }
    _handleData(data) {
        this._backend.parse(data, (code, payload) => {
            try {
                switch (code) {
                    case protocol_1.Protocol.BackendMessageCode.Authentication:
                        this._handleAuthenticationMessage(payload);
                        break;
                    case protocol_1.Protocol.BackendMessageCode.ErrorResponse:
                        this.emit('error', new DatabaseError_1.DatabaseError(payload));
                        break;
                    case protocol_1.Protocol.BackendMessageCode.NoticeResponse:
                        this.emit('notice', payload);
                        break;
                    case protocol_1.Protocol.BackendMessageCode.ParameterStatus:
                        this._handleParameterStatus(payload);
                        break;
                    case protocol_1.Protocol.BackendMessageCode.BackendKeyData:
                        this._handleBackendKeyData(payload);
                        break;
                    case protocol_1.Protocol.BackendMessageCode.ReadyForQuery:
                        if (this._state !== definitions_1.ConnectionState.READY) {
                            this._state = definitions_1.ConnectionState.READY;
                            this.emit('ready');
                        }
                        else
    ...
    _handleAuthenticationMessage(msg) {
        ...
        switch (msg.kind) {
            case protocol_1.Protocol.AuthenticationMessageKind.CleartextPassword:
                this._resolvePassword(password => {
                    this._send(this._frontend.getPasswordMessage(password));
                });
                break;
            case protocol_1.Protocol.AuthenticationMessageKind.MD5Password:
                this._resolvePassword((password => {
                ...
                break;
            case protocol_1.Protocol.AuthenticationMessageKind.SASL: {
                if (!msg.mechanisms.includes('SCRAM-SHA-256'))
                    throw new Error('SASL: Only mechanism SCRAM-SHA-256 is currently supported');
                const saslSession = this._saslSession =
                    sasl_1.SASL.createSession(this.options.user || '', 'SCRAM-SHA-256');
                this._send(this._frontend.getSASLMessage(saslSession));
                break;
            }
            case protocol_1.Protocol.AuthenticationMessageKind.SASLContinue: {
                const saslSession = this._saslSession;
                if (!saslSession)
                    throw new Error('SASL: Session not started yet');
                this._resolvePassword(password => {
                    sasl_1.SASL.continueSession(saslSession, password, msg.data);
                    const buf = this._frontend.getSASLFinalMessage(saslSession);
                    this._send(buf);
                });
                break;
            }
            case protocol_1.Protocol.AuthenticationMessageKind.SASLFinal: {
                const session = this._saslSession;
                if (!session)
                    throw new Error('SASL: Session not started yet');
                sasl_1.SASL.finalizeSession(session, msg.data);
                this._saslSession = undefined;
                break;
            }
        }
