...
class Connection extends SafeEventEmitter_1.SafeEventEmitter {
    ...
    async query(sql, options) {
        ...
        const paramTypes = options?.params?.map(prm => prm instanceof BindParam_1.BindParam ? prm.oid : typeMap.determine(prm) || definitions_1.DataTypeOIDs.varchar);
        const statement = await this.prepare(sql, { paramTypes, typeMap })
            .catch((e) => {
            throw this._handleError(e, sql);
        });
        try {
            const params = options?.params?.map(prm => prm instanceof BindParam_1.BindParam ? prm.value : prm);
